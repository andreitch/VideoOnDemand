    ...
    public static class HttpClientExtensions
    {
        private static HttpRequestMessage CreateRequestHeaders(this string uri, HttpMethod httpMethod, string token = "")
        {
            ...
            if (!token.IsNullOrEmptyOrWhiteSpace())
                requestHeader.Headers.Authorization = new AuthenticationHeaderValue("bearer", token);
        ...
        public static async Task<TResponse> GetAsync<TResponse, TRequest>(
            this HttpClient client, string uri, CancellationToken cancellationToken, TRequest content, string token = "")
        {
            try
            {
                var requestMessage = uri.CreateRequestHeaders(HttpMethod.Get, token);
